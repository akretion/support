...
from ..schemas.task import (
    TaskCreateInput,
    TaskGetMessageInput,
    TaskMessageFetchInput,
    TaskMessageFormatInput,
    TaskMessagePostInput,
    TaskReadGroupInput,
    TaskReadInput,
    TaskSearchInput,
    TaskWriteInput,
...
def connection_config(
    ...
    for project in env["project.project"].search(
        [("partner_id", "=", partner.id), ("customer_display", "=", True)]
    ):
        config["projects"].append(
            {
                ...
                "stages": [
                    {"id": st.id, "name": st.name, "sequence": st.sequence}
...
def attachment_download_url(
    ...
    if not attachment:
        ...
        url = (
            "%(base_url)s/web/content/?id=%(attach_id)s&download=true"
            "&filename=%(filename)s&access_token=%(token)s"
        ) % {
            "base_url": base_url,
    ...
    if tasks:
        for task in tasks:
            if "create_date" in task:
                task["create_date"] = fields.Datetime.to_string(task["create_date"])
            ...
            for key in [
                ...
                if key in task:
                    task[key] = env[
                        "support.task.api.helper"
                    ]._map_partner_read_to_data(task[key])
            if version < "2.0":
                if "tag_ids" in task:
...
def task_read_group(
    ...
    if group_by_stage_name or "stage_id" in groupby[0]:
        project_ids = env["support.task.api.helper"]._get_all_project_ids_from_domain(
            partner, domain
        )
...
def task_create(
    ...
    if assignee_customer_data:
        vals["assignee_customer_id"] = (
            env["support.task.api.helper"]
            ._get_partner(partner, assignee_customer_data.model_dump())
            .id
        )
        vals.pop("assignee_customer")
    author = env["support.task.api.helper"]._get_partner(
        partner, author_data.model_dump()
    )
    vals.pop("author")
...
def task_write(
    ...
    author = env["support.task.api.helper"]._get_partner(
        partner, data.author.model_dump()
    )
    tasks = env["project.task"].search(
        [("id", "in", data.ids), ("project_id.partner_id", "=", partner.id)]
    )
    ...
    if data.assignee_customer:
        vals["assignee_customer_id"] = (
            env["support.task.api.helper"]
            ._get_partner(partner, data.assignee_customer.model_dump())
            .id
        )
    if version < "2.0":
        if "tag_ids" in vals:
...
class SupportTaskApiHelper(models.AbstractModel):
    ...
    def _map_partner_read_to_data(self, partner_read):
        ...
        if (
            not partner_read
            or (isinstance(partner_read, tuple) and not partner_read[0])
            or isinstance(partner_read, list)
        ):
    ...
    def _get_all_project_ids_from_domain(self, partner, domain):
        ...
        all_project_ids = (
            self.env["project.project"].search([("partner_id", "=", partner.id)]).ids
        )
        project_ids = []
        for elem in domain:
